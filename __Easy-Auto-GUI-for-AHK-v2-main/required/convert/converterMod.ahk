...
classify(moddedOutput) {
    ...
    needle := ".*:=\s*Gui\(\).*"
    needleFound := 0
    lines := StrSplit(moddedOutput, "`n")
    Loop lines.Length
    {
        if needleFound = 0 {
            RegExMatch(lines[A_Index], needle, &match)
            if not IsObject(match){
                newReturnCode .= lines[A_Index] "`n"
            } 
            else {
                newReturnCode .= preludeString() "`n" "`t`t" lines[A_Index] 
                needleFound := 1
            }
        } 
        else {
            newReturnCode .= "`n" "`t`t" lines[A_Index] 
        }
    }
